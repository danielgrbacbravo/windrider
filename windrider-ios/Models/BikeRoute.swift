...
import CoreLocation
import SwiftUI
class BikeRoute: ObservableObject{
	...
	// init
	init(name: String? = nil, averageCoordinate: CLLocationCoordinate2D, coordinates: [CLLocationCoordinate2D]? = nil, coordinateAngles: [Int]? = nil, bikeRouteCondition: BikeRouteCondition? = nil, bikeRouteCoordinateCondition: [BikeRouteCoordinateCondition]? = nil) {
	...
	static private func findNorthRelativeAngle(from start: CLLocationCoordinate2D, to end: CLLocationCoordinate2D) -> Int {
	...
	static private func findAllNorthRelativeAngles(from coordinates: [CLLocationCoordinate2D]) -> [Int] {
		var angles: [Int] = []
	...
	static private func calculateAverageCoordinate(coordinates: [CLLocationCoordinate2D]) -> CLLocationCoordinate2D {
		...
		return CLLocationCoordinate2D(latitude: averageLat, longitude: averageLong)
	}
...
struct BikeRouteCondition{
	var windSpeed: Int
	var totalHeadwindPercentage: Int
	var totalCrosswindPercentage: Int
	var totalTailwindPercentage: Int
	
	init(windSpeed: Int, totalHeadwindPercentage: Int, totalCrosswindPercentage: Int, totalTailwindPercentage: Int) {
...
struct BikeRouteCoordinateCondition{
	// general wind data for a coordinate
	...
	init(coordinateAngle: Int, windSpeed: Double, windAngle: Int) {
		self.relativeWindAngle = BikeRouteCoordinateCondition.findRelativeWindAngle(coordinateAngle: coordinateAngle, windAngle: windAngle)
		self.windSpeed = windSpeed
		self.headWindPercentage = BikeRouteCoordinateCondition.findHeadwindPercentage(relativeWindAngle: self.relativeWindAngle)
		self.crossWindPercentage = BikeRouteCoordinateCondition.findCrosswindPercentage(relativeWindAngle: self.relativeWindAngle)
		self.tailWindPercentage = BikeRouteCoordinateCondition.findTailwindPercentage(relativeWindAngle: self.relativeWindAngle)
	}
